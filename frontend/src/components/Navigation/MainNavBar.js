...
const MainNavBar = () => {
  ...
  return (
    <nav class="flex items-center justify-between flex-wrap bg-blue-700 p-6">
      ...
      <div>{currentUser.token && coinIcon}</div>
      <div>
        {currentUser.token && (
          <div class="block lg:inline-block lg:mt-0 text-yellow-400 hover:text-white mr-4">
            {currentUser.coins}
          </div>
        )}
      </div>
      <div>{currentUser.token && userProfilePic}</div>
