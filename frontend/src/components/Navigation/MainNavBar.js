...
const MainNavBar = () => {
  ...
  const coinIcon = (
    <svg
        ...
        >
          <g>
            <circle cx="256" cy="256" fill="#ffe14d" r="256" />
            <path
            ...
            <g>
              <g>
                <path
                  d="m256 451c-107.52 0-195-87.48-195-195s87.48-195 195-195 195 87.48 195 195-87.48 195-195 195z"
                  fill="#fb3"
                />
              </g>
            </g>
  ...
  return (
    <nav class="flex items-center justify-between flex-wrap bg-blue-700 p-6">
      ...
      <div>{currentUser.token && coinIcon }
        
      </div>
      <div>
        {currentUser.token && <div class="block lg:inline-block lg:mt-0 text-yellow-400 hover:text-white mr-4">{currentUser.coins}</div>}
      </div>
      <div>{currentUser.token ? profileLink : loginLink}</div>
    </nav>
  );
};
